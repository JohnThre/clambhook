...
// TestDialThroughClosesUnderlying proves the OpenVPN stream decline path
...
func TestDialThroughClosesUnderlying(t *testing.T) {
	...
	c, err := d.DialThrough(context.Background(), tracked, "example.com:80")
	if err == nil {
		if c != nil {
			c.Close()
		}
...
// TestDialPacketThroughClosesUnderlying proves the OpenVPN packet decline
...
func TestDeclinePathsTolerateNilUnderlying(t *testing.T) {
	d := &dialer{cfg: &config{}}
	if _, err := d.DialThrough(context.Background(), nil, "example.com:80"); err == nil {
