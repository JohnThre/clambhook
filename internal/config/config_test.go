...
func TestValidateDeveloperRewriteRule(t *testing.T) {
	// A complete, valid rewrite rule must validate.
	validOp := func() DeveloperRewriteOp {
		return DeveloperRewriteOp{Target: "header", Action: "add", Field: "X-Test", Value: "dev"}
	}
	cases := []struct {
		name       string
		rule       DeveloperRewriteRuleConfig
		wantSubstr string
	}{
		{
			name:       "missing stage",
			rule:       DeveloperRewriteRuleConfig{ID: "r1", Stage: "", Ops: []DeveloperRewriteOp{validOp()}},
			wantSubstr: "developer.rewrite_rule 0 stage is required",
		},
		{
			name:       "bad stage",
			rule:       DeveloperRewriteRuleConfig{ID: "r1", Stage: "aftermath", Ops: []DeveloperRewriteOp{validOp()}},
			wantSubstr: "stage",
		},
		{
			name:       "no ops",
			rule:       DeveloperRewriteRuleConfig{ID: "r1", Stage: "both", Ops: nil},
			wantSubstr: "at least one op",
		},
		{
			name:       "bad target",
			rule:       DeveloperRewriteRuleConfig{ID: "r1", Stage: "both", Ops: []DeveloperRewriteOp{{Target: "url", Action: "set"}}},
			wantSubstr: "target",
		},
		{
			name:       "bad header action",
			rule:       DeveloperRewriteRuleConfig{ID: "r1", Stage: "both", Ops: []DeveloperRewriteOp{{Target: "header", Action: "replace", Field: "X"}}},
			wantSubstr: "header action",
		},
		{
			name:       "header missing field",
			rule:       DeveloperRewriteRuleConfig{ID: "r1", Stage: "both", Ops: []DeveloperRewriteOp{{Target: "header", Action: "remove"}}},
			wantSubstr: "field is required for header",
		},
		{
			name:       "header add missing value",
			rule:       DeveloperRewriteRuleConfig{ID: "r1", Stage: "both", Ops: []DeveloperRewriteOp{{Target: "header", Action: "add", Field: "X"}}},
			wantSubstr: "value is required for header add",
		},
		{
			name:       "body bad action",
			rule:       DeveloperRewriteRuleConfig{ID: "r1", Stage: "both", Ops: []DeveloperRewriteOp{{Target: "body", Action: "add"}}},
			wantSubstr: "body action",
		},
		{
			name:       "body set missing value",
			rule:       DeveloperRewriteRuleConfig{ID: "r1", Stage: "both", Ops: []DeveloperRewriteOp{{Target: "body", Action: "set"}}},
			wantSubstr: "value is required for body set",
		},
		{
			name:       "body replace missing replacement",
			rule:       DeveloperRewriteRuleConfig{ID: "r1", Stage: "both", Ops: []DeveloperRewriteOp{{Target: "body", Action: "replace", Value: "x"}}},
			wantSubstr: "replace (replacement) is required",
		},
		{
			name:       "status on request-only stage",
			rule:       DeveloperRewriteRuleConfig{ID: "r1", Stage: "request", Ops: []DeveloperRewriteOp{{Target: "status", Action: "set", Value: "204"}}},
			wantSubstr: "status op is only valid when stage is response or both",
		},
		{
			name:       "status bad action",
			rule:       DeveloperRewriteRuleConfig{ID: "r1", Stage: "both", Ops: []DeveloperRewriteOp{{Target: "status", Action: "add", Value: "204"}}},
			wantSubstr: "status action",
		},
		{
			name:       "status non-numeric",
			rule:       DeveloperRewriteRuleConfig{ID: "r1", Stage: "both", Ops: []DeveloperRewriteOp{{Target: "status", Action: "set", Value: "ok"}}},
			wantSubstr: "must be an integer",
		},
		{
			name:       "status out of range",
			rule:       DeveloperRewriteRuleConfig{ID: "r1", Stage: "both", Ops: []DeveloperRewriteOp{{Target: "status", Action: "set", Value: "42"}}},
			wantSubstr: "between 100 and 599",
		},
		{
			name:       "id required",
			rule:       DeveloperRewriteRuleConfig{ID: "", Stage: "both", Ops: []DeveloperRewriteOp{validOp()}},
			wantSubstr: "developer.rewrite_rule 0 id is required",
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			cfg.Developer.RewriteRules = []DeveloperRewriteRuleConfig{tc.rule}
			err := cfg.Validate()
	...
	// A fully valid rule must pass.
	t.Run("valid rule", func(t *testing.T) {
		cfg := validConfig()
		cfg.Developer.Enabled = true
		cfg.Developer.RewriteRules = []DeveloperRewriteRuleConfig{{
			ID: "rw-1", Name: "rewrite", Enabled: true, Stage: "both",
			Match: DeveloperMatchConfig{Host: "api.example.com", PathPrefix: "/v1/"},
			Ops: []DeveloperRewriteOp{
