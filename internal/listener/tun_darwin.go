...
// TUN is a macOS device-wide ingress listener backed by a utun interface and
...
func (t *TUN) Start(parent context.Context) error {
	...
	routeMgr := newDarwinRouteManager(name, mtu, t.opts, t.ch)
	...
	stack := NewPacketStack(t.opts, t.ch, t.planner, darwinTUNPacketWriter{dev: dev})
...
func (t *TUN) tunToStackLoop(ctx context.Context, dev tun.Device, stack *PacketStack, mtu int) {
	...
	for i := range bufs {
		bufs[i] = make([]byte, mtu+128+darwinTUNOffset)
	}
	...
	for {
		...
		n, err := dev.Read(bufs, sizes, darwinTUNOffset)
		if err != nil {
		...
		for i := 0; i < n; i++ {
			packet := bufs[i][darwinTUNOffset : darwinTUNOffset+sizes[i]]
...
type darwinTUNPacketWriter struct {
	dev tun.Device
}

func (w darwinTUNPacketWriter) WritePacket(pkt []byte) error {
	buf := make([]byte, len(pkt)+darwinTUNOffset)
	copy(buf[darwinTUNOffset:], pkt)
	_, err := w.dev.Write([][]byte{buf}, darwinTUNOffset)
	return err
}
