...
// This file exposes the shared license domain (internal/license) to mobile
// clients through gomobile. Kotlin owns persistence (encrypted store + DataStore)
// and passes the cached snapshot JSON in; the bridge performs evaluation, the
// store.swiphtgroup.com HTTP calls, and the "apply server response" transform,
// returning JSON for Kotlin to persist and render.
