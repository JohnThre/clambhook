...
static ch_status mutation_normalize_name_array(
    const ch_json_value *request, const char *key,
    const char *const *fallback, size_t fallback_count,
    ch_json_value **out_array, ch_error *error) {
    *out_array = NULL;
    const ch_json_value *values = ch_json_object_get(request, key);
    if (values == NULL || ch_json_value_type(values) == CH_JSON_NULL) {
        return CH_OK;
    }
    if (ch_json_value_type(values) != CH_JSON_ARRAY) {
        return mutation_type_error(key, "an array", error);
    }
    ch_json_value *result = ch_json_value_new_array();
    if (result == NULL) {
        ch_error_set(error, CH_ERROR_OUT_OF_MEMORY, "allocate %s", key);
        return CH_ERROR_OUT_OF_MEMORY;
    }
    size_t count = ch_json_array_size(values);
    if (count == 0U) count = fallback_count;
    for (size_t index = 0U; index < count; ++index) {
        const char *source = ch_json_array_size(values) == 0U ?
            fallback[index] : ch_json_string_value(
                ch_json_array_get(values, index));
        if (source == NULL) {
            ch_json_value_destroy(result);
            return mutation_type_error(key, "an array of strings", error);
        }
        char *name = mutation_trimmed_copy(source);
        if (name == NULL) {
            ch_json_value_destroy(result);
            ch_error_set(error, CH_ERROR_OUT_OF_MEMORY, "copy %s", key);
            return CH_ERROR_OUT_OF_MEMORY;
        ...
        if (name[0] == '\0') {
            ...
            ch_json_value_destroy(result);
            ch_error_set(error, CH_ERROR_INVALID_ARGUMENT,
                         "%s[%zu] must not be empty", key, index);
            return CH_ERROR_INVALID_ARGUMENT;
        }
        ch_json_value *item = ch_json_value_new_string(name);
        free(name);
        if (item == NULL || ch_json_array_append(result, item, error) != CH_OK) {
            ch_json_value_destroy(item);
            ch_json_value_destroy(result);
            if (error->code == CH_OK) {
                ch_error_set(error, CH_ERROR_OUT_OF_MEMORY, "append %s", key);
            }
            return error->code;
        }
    }
...
static ch_status mutation_apply_developer_settings(ch_json_value *root,
                                                   const ch_json_value *request,
                                                   ch_error *error) {
    static const char *const redact_headers[] = {
        "authorization", "proxy-authorization", "cookie", "set-cookie",
        "x-api-key", "api-key", "x-auth-token", "x-csrf-token",
        "x-xsrf-token", "csrf-token", "xsrf-token"
    };
    static const char *const redact_query[] = {
        "token", "access_token", "refresh_token", "id_token", "api_key",
        "apikey", "key", "secret", "password", "passwd", "code",
        "session", "auth"
    };
    ch_json_value *developer = mutation_ensure_object(root, "developer", error);
    if (developer == NULL) return error->code;
    bool was_enabled = ch_json_bool_value(
        ch_json_object_get(developer, "enabled"), false);
    bool was_mitm = ch_json_bool_value(
        ch_json_object_get(developer, "mitm_enabled"), false);
    const ch_json_value *enabled = ch_json_object_get(request, "enabled");
    const ch_json_value *mitm = ch_json_object_get(request, "mitm_enabled");
    const ch_json_value *ack = ch_json_object_get(request,
                                                  "https_capture_ack");
    if (ack != NULL && ch_json_value_type(ack) != CH_JSON_NULL &&
        ch_json_value_type(ack) != CH_JSON_BOOL) {
        return mutation_type_error("https_capture_ack", "a boolean", error);
    }
    if (enabled != NULL && ch_json_value_type(enabled) != CH_JSON_NULL) {
        if (ch_json_value_type(enabled) != CH_JSON_BOOL) {
            return mutation_type_error("enabled", "a boolean", error);
        }
        bool next_enabled = ch_json_bool_value(enabled, false);
        if (mutation_set_clone(developer, "enabled", enabled, error) != CH_OK) {
            return error->code;
        }
        if (!next_enabled || mitm == NULL ||
            ch_json_value_type(mitm) == CH_JSON_NULL) {
            if (mutation_set_owned(developer, "mitm_enabled",
                                   ch_json_value_new_bool(false), error) !=
                CH_OK) return error->code;
        }
    }
    bool next_enabled = ch_json_bool_value(
        ch_json_object_get(developer, "enabled"), false);
    if (mitm != NULL && ch_json_value_type(mitm) != CH_JSON_NULL) {
        if (ch_json_value_type(mitm) != CH_JSON_BOOL) {
            return mutation_type_error("mitm_enabled", "a boolean", error);
        }
        bool next_mitm = ch_json_bool_value(mitm, false);
        if (next_mitm && !next_enabled) {
            ch_error_set(error, CH_ERROR_INVALID_ARGUMENT,
                         "developer capture must be enabled before HTTPS capture");
            return CH_ERROR_INVALID_ARGUMENT;
        }
        if (next_mitm && !(was_enabled && was_mitm) &&
            !ch_json_bool_value(ack, false)) {
            ch_error_set(error, CH_ERROR_INVALID_ARGUMENT,
                         "https_capture_ack is required when enabling HTTPS capture");
            return CH_ERROR_INVALID_ARGUMENT;
        }
        if (mutation_set_clone(developer, "mitm_enabled", mitm, error) !=
            CH_OK) return error->code;
    }
    const ch_json_value *no_cache = ch_json_object_get(
        request, "no_cache_enabled");
    if (no_cache != NULL && ch_json_value_type(no_cache) != CH_JSON_NULL) {
        if (ch_json_value_type(no_cache) != CH_JSON_BOOL) {
            return mutation_type_error("no_cache_enabled", "a boolean", error);
        }
        if (mutation_set_clone(developer, "no_cache_enabled", no_cache,
                               error) != CH_OK) return error->code;
    }
    const char *limits[] = {
    ...
    for (size_t index = 0U; index < 3U; ++index) {
        const ch_json_value *value = ch_json_object_get(request, limits[index]);
        if (value == NULL || ch_json_value_type(value) == CH_JSON_NULL) continue;
        double number = ch_json_number_value(value, -1.0);
        if (ch_json_value_type(value) != CH_JSON_NUMBER || number < 0.0 ||
            floor(number) != number) {
            return mutation_type_error(limits[index],
                                       "a non-negative integer", error);
        }
        if (mutation_set_clone(developer, limits[index], value, error) !=
            CH_OK) return error->code;
    }
    const struct {
        const char *key;
        ...
        {"redact_headers", redact_headers,
         sizeof(redact_headers) / sizeof(redact_headers[0])},
        {"redact_query_params", redact_query,
         sizeof(redact_query) / sizeof(redact_query[0])},
        {"ssl_decrypt_hosts", NULL, 0U}
    };
    for (size_t index = 0U; index < 3U; ++index) {
        ch_json_value *array = NULL;
        ch_status status = mutation_normalize_name_array(
            request, arrays[index].key, arrays[index].fallback,
            arrays[index].fallback_count, &array, error);
        if (status != CH_OK) return status;
        if (array != NULL && mutation_set_owned(
                developer, arrays[index].key, array, error) != CH_OK) {
            return error->code;
        }
    }
    return CH_OK;
