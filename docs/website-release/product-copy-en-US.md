...
surface. It is distributed as a direct download from
`store.clambercloud.com`; checkout, license delivery, and device-seat
...
breakpoints, map rules, repeat/compose, and HAR export. Profile data,
credentials, keys, connection metadata, logs, captures, and diagnostics stay on
the device unless the user explicitly exports or sends them.
