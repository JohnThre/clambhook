...
private class FakeApi : ClambhookApi {
    ...
    override suspend fun createRule(rule: RulePayload): RulesPayload {
        actions.add("rule:${rule.name}")
        rulesPayload =
            rulesPayload.copy(rules = rulesPayload.rules + rule)
        return rulesPayload
    }
    ...
    override suspend fun createRuleFromConnection(
        ...
        actions.add("connection-rule:${connection.connId}:${rule.name}")
        rulesPayload =
            rulesPayload.copy(rules = rulesPayload.rules + rule)
        return rulesPayload
    }
    ...
    override suspend fun cleanupRule(suggestion: TrafficCleanupSuggestionPayload): RulesPayload {
        actions.add("cleanup:${suggestion.operation}:${suggestion.targetRuleName}")
        return rulesPayload
    }
    ...
    override suspend fun selectPolicyGroup(
        ...
        actions.add("policy-select:$group:$chain")
        return PolicyGroupsPayload()
    }

    override suspend fun testPolicyGroups(group: String): PolicyGroupsPayload {
        actions.add("policy-test:$group")
        return PolicyGroupsPayload()
    }
    ...
    override suspend fun resolvePrompt(
        ...
        actions.add("prompt:$id:$action:$scope")
    }
    ...
    override suspend fun updateConditioner(request: ConditionerUpdateRequest): ConditionerPayload {
        actions.add("conditioner:${request.enabled}")
...
class DashboardStoreTest {
    @Test
    fun refreshLoadsStatusProfilesAndServers() =
        runBlocking {
            val api = FakeApi()
            api.statusPayload =
                ApiJson.decodeFromString(
                    StatusPayload.serializer(),
                    """{"running":true,"profile":"A","listeners":[{"protocol":"socks5","addr":"127.0.0.1:1080","active_conns":3}]}""",
                )
            api.profilesPayload = ApiJson.decodeFromString(ProfilesPayload.serializer(), """{"profiles":["A","B"],"active":"A"}""")
            api.serversPayload =
                ApiJson.decodeFromString(
                    ServersPayload.serializer(),
                    """{"profile":"A","chains":[{"name":"default","servers":[{"name":"london","address":"uk.example:443","protocol":"clambback"}]}]}""",
                )
            api.rulesPayload =
                ApiJson.decodeFromString(
                    RulesPayload.serializer(),
                    """{"profile":"A","rules":[{"name":"ads","action":"block","domains":["ads.example.com"]}]}""",
                )
            api.trafficPayload =
                ApiJson.decodeFromString(
                    TrafficSnapshotPayload.serializer(),
                    """{"summary":{"active_connections":1,"rx_bps":2048},"connections":[{"conn_id":"c1","state":"active","target":"example.com:443"}]}""",
                )
            ...
            assertEquals(
                "london",
                store.state.value.servers.chains[0]
                    .servers[0]
                    .name,
            )
            assertEquals(
                "ads",
                store.state.value.rules.rules[0]
                    .name,
            )
            assertEquals(
                "example.com:443",
                store.state.value.traffic.connections[0]
                    .target,
            )
        }

    @Test
    fun eventRateAndLogRetention() =
        runBlocking {
            ...
            val updatedConn =
                store.state.value.traffic.connections
                    .firstOrNull { it.connId == "c1" }
            ...
            for (i in 0 until 205) {
                store.applyEvent(
                    DaemonEvent(type = "log.line", data = mapOf("line" to kotlinx.serialization.json.JsonPrimitive("line-$i"))),
                )
            }
    ...
    @Test
    fun createRuleRefreshesDashboard() =
        runBlocking {
            val api = FakeApi()
            val store = DashboardStore(api)
