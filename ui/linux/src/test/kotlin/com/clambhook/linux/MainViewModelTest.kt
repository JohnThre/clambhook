...
class MainViewModelTest {
    private class FakeApi : ClambhookApi {
        ...
        override suspend fun createRuleFromConnection(connection: TrafficConnectionPayload, rule: RulePayload): RulesPayload = RulesPayload()
        ...
        override suspend fun selectPolicyGroup(group: String, chain: String): PolicyGroupsPayload = PolicyGroupsPayload()
        ...
        override fun configureBaseUrl(baseUrl: String) { configuredBaseUrl = baseUrl }
    }
    ...
    private class FakeTokenVault : TokenVault {
        ...
        override suspend fun saveToken(token: String) { savedToken = token; savedCount++ }
    }
    ...
    private class FakeLicenseHelper : LicenseHelperClient("") {
        override suspend fun call(command: String, request: kotlinx.serialization.json.JsonObject): String = ""
    }
    ...
    @Test
    fun saveSettingsAppliesCurrentStateImmediately() = runTest {
        val tokenRef = AtomicReference("")
        val api = FakeApi()
        val tmp = java.nio.file.Files.createTempDirectory("vm-test")
    ...
    @Test
    fun saveSettingsRejectsEndpointWithPathAndFallsBack() = runTest {
        val tokenRef = AtomicReference("")
        val api = FakeApi()
        val tmp = java.nio.file.Files.createTempDirectory("vm-test")
        val settingsStore = FileSettingsStore(tmp.resolve("settings.json"))
    ...
    @Test
    fun saveSettingsStoresCurrentTokenImmediately() = runTest {
        val tokenRef = AtomicReference("")
        val api = FakeApi()
        val vm = createViewModel(
            tokenRef,
            api,
            object : FileSettingsStore() { override fun save(settings: AppSettings) {} },
            object : TokenVault {
    ...
    fun baseUrlReconfigurationIsAtomic() {
        ...
        assertEquals("proxy.example:9090", client.eventsUri().removePrefix("ws://").removeSuffix("/api/v1/events?types=connection.*,rule.*,hop.*,log.*"))
    }
