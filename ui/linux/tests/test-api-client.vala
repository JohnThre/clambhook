namespace Clambhook.Tests {
    public void add_api_client_tests() {
        Test.add_func("/linux/api-client/normalizes-urls-and-builds-event-url", () => {
            ...
            assert_cmpstr(client.events_uri(), CompareOperator.EQ, "ws://127.0.0.1:9090/api/v1/events?types=connection.*,log.*");

            var tls = new ClambhookApiClient("https://proxy.example.test/", () => "");
            assert_cmpstr(tls.events_uri(), CompareOperator.EQ, "wss://proxy.example.test/api/v1/events?types=connection.*,log.*");
        });
